...
class SSA:
    def __init__(self, func, n_dim=None, pop_size=20, max_iter=50, lb=-512, ub=512, verbose=False):
        ...
        self.X = [np.random.choice(self.n_dim, self.n_dim, False) for i in range(self.pop)]

        self.Y = [self.func(self.X[i]) for i in range(len(self.X))]  # y = f(x) for all particles
        print(self.Y)
        self.pbest_x = self.X.copy()  # personal best location of every particle in history
        self.pbest_x = np.array(self.pbest_x)
    ...
    def run(self, max_iter=None):
        ...
        self.best_x = np.array(self.best_x)
        self.best_x = np.argsort(self.best_x)
...
def func_tsp(x):
    ...
    for i in range(len(x) - 1):
        temp += distance_graph[x[i]][x[i + 1]]
    temp += distance_graph[0][-1]
    return temp
